        ...
        function addAutovisorAccount() { if (!state.settings) { showToast('请先等待配置加载', 'error'); return; } state.settings.autovisor.accounts.push(normalizeAutovisorAccount({}, state.settings.autovisor.accounts.length + 1)); renderAutovisorAccounts(state.settings.autovisor.accounts); }
